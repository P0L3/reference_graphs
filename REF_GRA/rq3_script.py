...
def rq3_process_graph(multiplex_graph):
    """
    Analyses interdisciplinary brokerage and aggregation cost for a given
    multiplex graph dictionary. Graphs should have 'weight' edge attributes
    pre-cast to float before being passed in.

    Returns a tuple of:
        - df_rq3       : Full DataFrame with per-node metrics (all nodes)
        - df_internal  : Filtered DataFrame (active, non-External nodes) with ranks
        - stats        : dict with keys 'rho', 'p_value', 'information_loss_pct'
    """
    ...
    G_agg = nx.Graph()
    G_supra = nx.DiGraph()  # Directed to safely accommodate both layer types
    all_nodes = set()
    ...
    pr_multi = {
        node: sum(pr_supra.get(f"{node}_{l}", 0.0) for l in layer_names)
        for node in all_nodes
    }
    ...
    for node in all_nodes:
        ...
        # Participation Coefficient: P = 1 - sum((k_l / o_i)^2)
        k_layers = []
        for layer_name, G in multiplex_graph.items():
        ...
        o_i = sum(k_layers)
        participation = 1.0 - sum((k / o_i)**2 for k in k_layers) if o_i > 0 else 0.0
        ...
        rq3_data.append({
            ...
            "Constraint":   constraints.get(node, np.nan),
            "Total Degree": o_i
        })
    ...
    stats = {
        "rho":                 round(rho, 4),
        "p_value":             p_val,
        "information_loss_pct": round(information_loss, 1)
    }
    ...
    return df_rq3, df_internal, stats
...
if __name__ == "__main__":
    ...
    # Run the analysis
    df_all, df_ranked, stats = rq3_process_graph(multiplex_graph)

    print("\n==========================================")
    print("  RQ3 STATS SUMMARY")
    print("==========================================\n")
    print(f"  Spearman rho          : {stats['rho']}")
    print(f"  p-value               : {stats['p_value']:.2e}")
    print(f"  Information Loss (%)  : {stats['information_loss_pct']}%")