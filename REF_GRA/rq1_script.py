...
def rq1_process_graph(multiplex_graph, null_iterations=NULL_MODEL_ITERATIONS):
    """
    Calculates baseline topologies, homophily (assortativity), and performs 
    null-model statistical testing for a given multiplex graph dictionary.
    ...
    for layer_name, G in multiplex_graph.items():
        ...
        for _ in tqdm(range(null_iterations), desc=f"  Null Models ({layer_name})", leave=False):
...
if __name__ == "__main__":
    ...
    # Run the function
    df_results = rq1_process_graph(multiplex_graph)
